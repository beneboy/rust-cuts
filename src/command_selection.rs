...
pub fn read_option_input(
    ...
    loop {
        let rerun_text = if last_command_parameters.is_some() {
            format!(", or `{LAST_COMMAND_OPTION}` to re-run last")
        } else {
            String::new()
        };
        // Prompt the user for input
        print!(
            "Enter an option (0-{}{}. Quit with `q`): ",
            max - 1,
            rerun_text
        );
        io::stdout().flush()?;
        ...
        if input == "r" {
            match last_command_parameters {
                ...
                Some(last_command_parameters) => {
                    return Ok(CommandChoice::Rerun(last_command_parameters.clone()));
                }
            }
        }
        ...
        match input.parse::<usize>() {
            Ok(index) => {
                if index < max {
                    return Ok(CommandChoice::Index(index));
                }
...
pub fn prompt_for_command_choice(
    ...
    if let Some(last_command) = last_command.clone() {
        ...
        let interpolated_last_command =
            interpolate_command(&template_context, &get_templates(&last_command.command)?)?;
        println!(
            "{}",
